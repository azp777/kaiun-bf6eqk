...
/* ================================================================
   5. 運命星めぐり(簡易版)
================================================================ */
const ROKUSEI_STARS=['土星','金星','火星','天王星','木星','水星'];
const ROKUSEI_DESC={
 '土星':'理想を追う努力家。誠実で忍耐強く、信念を曲げない生き方が周囲の尊敬を集めます。',
 '金星':'感性で生きる自由人。楽しいこと・美しいものに敏感で、天性の華やかさがあります。',
 '火星':'独自の世界を持つ個性派。直感と集中力に優れ、ここぞという場面で勝負強さを発揮します。',
 '天王星':'人懐こい人情家。誰とでも打ち解ける柔らかさがあり、人に恵まれる星です。',
 '木星':'品格と正義感の人。曲がったことが嫌いで、コツコツ積み上げた信用が財産になります。',
 '水星':'頭の回転が速い合理主義者。時代の流れを読む才覚があり、お金や情報に強い星です。'
};
const ROKUSEI_SEED={'土星':0,'金星':2,'火星':4,'天王星':6,'木星':8,'水星':10};
const PHASES=[
 ['種まき','新しいサイクルの始まり。種まきに最適',3],
 ['芽吹き','蒔いた種が芽吹く成長期。学びが実る',3],
 ['開花','花開く絶好調期。積極的な行動が吉',3],
 ['小休み','小休止。健康第一で無理をしない',2],
 ['実り','最高の収穫期。勝負をかける年',3],
 ['ゆらぎ','心が揺れやすいゆらぎ期。現状維持が吉',1],
 ['めぐり','運気回復。過去の縁や経験が活きる',3],
 ['豊穣','金運上昇期。実利を固める好機',3],
 ['安寧','安定した充実期。将来の計画を立てる',3],
 ['冬入り','冬運期1年目。新規のことは控えめに',0],
 ['冬ごもり','冬運期2年目。守りに徹し、内面を磨く',0],
 ['夜明け前','冬運期3年目。出口は近い。整理と充電を',0]
];
...
const CHAT_CHIPS=['今日の運勢は?','今月の吉方位は?','おすすめの旅行先は?','恋愛運は?','仕事運は?','金運アップの方法は?','運気の冬はいつ?','私に合うパワースポットは?','相性を教えて'];
function initChat(){
...
function botReply(q){
  ...
  if(/大殺界|殺界|冬運|運気の冬/.test(q)){
    const idx=c.roku.phaseIdx,y=c.roku.nowYear;
    const start=y+((9-idx)+12)%12;
    if(idx>=9)return `${c.roku.star}タイプの${name}は、実は今(${y}年)「${PHASES[idx][0]}」で冬運期の${idx-8}年目です。でも怖がりすぎないで。新しい大勝負を避けて「学び・整理・体調管理」に徹すれば、${y+(12-idx)}年からの上昇気流にきれいに乗れますよ✦`;
    if(idx===5)return `${y}年は「ゆらぎ」の年。心が揺れやすい年なので、大きな決断はいつもより慎重に。次の冬運期は${start}年〜${start+2}年です。それまでに攻めの種まきを✦`;
    return `${c.roku.star}タイプの${name}の次の冬運期は${start}年〜${start+2}年の3年間。今年は「${PHASES[idx][0]}」(${PHASES[idx][1]})なので、${start}年が来る前にしっかり動いておくのが吉です✦`;
  }
  if(/相性/.test(q)){
  ...
  if(/仕事|転職|勉強|試験|資格|昇進|キャリア/.test(q)){
    let s=`今日の仕事・学び運は${starBar(d.work)}。\n数秘術では${c.num.pyYear}年のあなたはパーソナルイヤー「${c.num.py}」。${PERSONAL_YEAR[c.num.py]}\n${PHASES[c.roku.phaseIdx][2]===0?'運命星めぐりでは冬運期中なので、転職などの大きな決断は情報収集をいつもより丁寧に。実力を磨く時期と割り切ると最強です。':'運命星めぐりの今年の運気は「'+PHASES[c.roku.phaseIdx][0]+'」。'+PHASES[c.roku.phaseIdx][1]+'。'}`;
  ...
  if(/金運|お金|宝くじ|貯金|投資|臨時収入/.test(q)){
    ...
    let s=`今日の金運は${starBar(d.money)}。ラッキーナンバーは「${d.num}」です✦\n運命星めぐりでは${zaiY===c.roku.nowYear?'なんと今年が金運の「豊穣」イヤー!実利を固める行動が吉です。':zaiY+'年が金運の「豊穣」イヤー。'}\nお財布の整理と玄関の掃除は、どの占術でも共通する金運アップの基本ですよ。`;
    if(typeof lunaMoney==='function'){const x=lunaMoney(c);if(x)s+='\nライフパス'+c.num.lp+'の'+name+'は、'+x;}
    return s;
  }
  if(/健康|体調|疲れ|睡眠/.test(q)){
    let s=`今日の健康運は${starBar(d.health)}。\n${d.health<=2?'今日は無理は禁物。睡眠を最優先して、温かいものを食べてくださいね。':'調子は悪くありません。良い習慣をひとつ足すのに向いた日です。'}${PHASES[c.roku.phaseIdx][0]==='小休み'?'\n運命星めぐりでは今年は「小休み」の年。健康診断と休息をいつもより大切に。':''}`;
  ...
  if(/パワースポット|神社|属性|参拝|お参り|お寺/.test(q)){
    if(c.zoku.unknown)return `神社属性を占うには血液型が必要です。上の「変更」ボタンから設定してくださいね。ちなみに生年月日だけで出せる基本数は「${c.zoku.base}」です。`;
  ...
  if(/マヤ|kin|紋章|ツォルキン/i.test(q)){
    if(c.maya.special)return `${name}は2月29日生まれ。マヤ暦では「フナブ・クの日」という、KINを持たない特別な存在です✦`;
  ...
  if(/使い方|何ができる|ヘルプ|help/i.test(q))
    return `私に聞けるのは…「今日の運勢」「吉方位・旅行先・引っ越し」「恋愛運・仕事運・金運・健康運」「運気の冬はいつ?」「パワースポット」「相性」「マヤ暦・数秘術・性格タイプの解説」など✦ 下のボタンからもどうぞ。`;
  const oracle=[
...
function aishouItems(me,pt){
  ...
  const rk={
   ...
   3:[64,'正反対の運気周期','あなたの冬運期の時期に相手は絶頂期、その逆もまた然り。どちらかが沈む時、必ずどちらかが支えられる——実はとても心強い縁です。']
  }[dd];
  items.push({sys:'運命星',icon:'⭐',score:rk[0],title:rk[1],text:`あなたは${rka.star}タイプ${rka.sign.slice(0,1)}、${nameB}は${rkb.star}タイプ${rkb.sign.slice(0,1)}。${rk[2]}`});
  /* 神社属性 */
